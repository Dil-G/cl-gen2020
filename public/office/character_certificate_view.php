...
?>
    ...
    <head>
        ...
        <title>Character Certificate <?php echo $userID ?></title>
        ...
        <style>
            * {
            ...
            body {
                ...
                width: 95%;
                height: 1096.0629px;
                margin: 200.38px 151.18px 113.38px 251.18px;
            }

            .content{
                margin-left: 300px;
                margin-top: 100px;
                border: #24344b solid 1px;
                margin-bottom: 20px;
            }
            ...
            .navbar {
                ...
                position: fixed;
                z-index: 10;
                box-shadow: 0 2px 0px 0px #9da0a4;
            ...
            .wrap img {
                ...
                color: #fff;
                float: left;
                margin: 30px 10px 5px 45px;
                border-bottom: solid 1px #60666b;
            }
            ...
            .wrap {
                ...
                padding: 0px;
                font-size: 15px;
                box-shadow: 0 10px 5px 0 #60666b;
                overflow-y: scroll;
...
<div class="content">
        ...
        FROM
             student st
             LEFT JOIN alresults ar on st.admissionNo = ar.studentID
        WHERE 
        st.admissionNo = '$userID'";

            $sql1 = "SELECT csports.*,sports_achievements.* FROM sports_achievements
        LEFT JOIN csports on csports.SportID = sports_achievements.categoryID
        WHERE studentID='$userID' ORDER BY impValue ASC";

            $sql2 = "SELECT csocieties.*,societies_achievements.* FROM societies_achievements
        LEFT JOIN csocieties on csocieties.SocietyID = societies_achievements.categoryID
        WHERE studentID='$userID' ORDER BY impValue ASC";

            $result_sql1 = mysqli_query($connect, $sql1);
            ...
            <table>
                ...
                <tr>
                    <td></td>
                    <td>G.C.E. (Advanced Level) Examination
            ...
            <?php
            $c = 0;
            while ($rows = mysqli_fetch_array($result_sql2)) {
                $c++;
                if ($c == 6) {
                    break;
                }
            ?> <tr>
                    <td></td>
                    <td>
                        <?php echo $rows["achievementName"] . " - Position " . $rows["position"] . " - " . date("Y", strtotime($rows["achievementDate"]));  ?>
                    </td>
                </tr>
            <?php } ?>
            ...
            <?php
            $c = 0;
            while ($rows = mysqli_fetch_array($result_sql1)) {
                $c++;
                if ($c == 6) {
                    break;
                }
