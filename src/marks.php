    ...
    if ($_FILES['file']['name']) {
        ...
        if ($filename[1] == 'csv') {
            ...
            while ($data = fgetcsv($handle)) {
                ...
                $query = "INSERT into marks(markID,teacherID, admissionNumber, subjectID,mark,classID) 
                ...
                if ($import_result) {
                    ...
                    header('Location: ../public/teacher/Tcr_class.php?Ggrades=' . $error);
                }
                $maxID = $maxID + 1;
            }
            fclose($handle);
