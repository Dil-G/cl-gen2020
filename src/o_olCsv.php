...
if (isset($_POST["submit"])) {
    ...
    $retrieve = "SELECT * FROM ol_results";
    ...
    if ($_FILES['file']['name']) {
        ...
        if ($filename[1] == 'csv') {
            ...
            while ($data = fgetcsv($handle)) {
              ...
              //  $query = "INSERT into ol_results (markID,studentID, subjectID, streamID,grade,examID) 
                //values('$MID','$item2','$item3','$item4','$item5','$item6')";

      $query = "INSERT into ol_results (examID,admissionNo, studentIndex, studentName,SubName,SubID,Grade) VALUES ('$item1','$item2','$item3','$item4','$item5','$item6','$item7')";
          mysqli_query($conn, $query);
                ...
                if ($import_result) {
                    ...
                    header('Location: ../public/office/o_olCsv.php?Ggrades=' . $error);
                }
                
            }
            fclose($handle);
            //echo "<script>alert('Import done');</script>";
            header('Location: ../public/office/o_olCsv.php');
        }
    }
}
...
//         $sql = "INSERT into ol_results (examID,admissionNo, studentIndex, studentName,SubName,CatName,Grade) VALUES (('$item1','$item2','" .$item3. "','" . $item4. "','$item5','$item6','$item7')";
